...
data = sorted(flatten_list(datas), key= lambda datas: datas[0])
count = 0
prev_N = data[0][0]
extracted_data = []
temp_data_holder_l = []
for datapiece in data:
    ...
    if current_N != prev_N:
        extracted_data.append(np.array(temp_data_holder_l))
        temp_data_holder_l = []
    mask = np.array([i & 0b10000 for i in datapiece[1]])
    paths = np.array(datapiece[1])
    l_length = paths[mask == 16].shape[0]


    temp_data_holder_l.append(l_length)
    prev_N = current_N



extracted_data = np.array(extracted_data)
np.save("greedy_e_curved.npy", extracted_data, allow_pickle=True)
