...
def numba_edges(nodes, r, r_s):
    ...
    for i in range(len(nodes)):
        ...
        for j in range(i + 1, n):
            node2 = nodes[j]
            dx = node2 - node1
            r_met = (node1[1] + node2[1]) / 2
            metric = np.array([[- (1 - r_s / r_met), r_s / r_met],
                               [r_s / r_met, (1 - r_s / r_met)]])
...
class Network:
    ...
    def generate(self):
        ...
        t_poses = np.random.uniform(0, 25, size= (self.n - 2))
        square_poses = np.dstack((t_poses, r_poses))[0].astype(np.float32)
        source_sink = np.array([[0, 40], [25, 20]])
