...
def fit_1(x, par):
    return 1 / (par[0] * np.sqrt(x)) + par[1] / x + par[2] / x**2 + par[3] / x**3
...
start_fit = int(len(n_range) * 1 / 15000)

least_squares_fit_2 = LeastSquares(n_range[start_fit:], (1 / l_means)[start_fit:],
                                 (l_err / l_means**2)[start_fit:], fit_2)
...
fig = plt.figure(figsize = (12, 10), dpi=200) # set a figure size similar to the default
ax = fig.add_subplot(111)
ax.grid()

xs = np.linspace(95, 15100, 2000)
ax.plot(xs, fit_1(xs, m_fit_1.values), "b--", linewidth=1.5,
        label=r'Fit: $\langle L \rangle^{-1} = m N^{-\frac{1}{2}}$'
              r'$ + \dfrac{a}{N} + \dfrac{b}{N^2} + \dfrac{c}{N^3}$' +
